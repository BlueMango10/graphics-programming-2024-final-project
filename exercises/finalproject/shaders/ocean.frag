...
void main()
{
	//vec3 albedo = Color.rgb * texture(NormalMap, TexCoord).rgb;
	vec3 albedo = Color.rgb;
	vec3 viewDirection = normalize(CameraPosition - WorldPosition);

	FragColor = vec4(blinnPhong(albedo, normalize(LightDirection), viewDirection, normalize(getCombinedAnimatedNormal())), Color.a);
